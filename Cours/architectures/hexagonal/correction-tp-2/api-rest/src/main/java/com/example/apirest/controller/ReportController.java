...
import com.example.apirest.dto.ReportRequestDTO;
import com.example.apirest.dto.ReportResponseDTO;
...
public class ReportController {
    ...
    @GetMapping("")
    public ResponseEntity<List<ReportResponseDTO>> get() {
        return ResponseEntity.ok(convertToListReports(reportService.getAllReports()));
    }
    @GetMapping("/{id}")
    public ResponseEntity<ReportResponseDTO> getById(@PathVariable int id) {
        return ResponseEntity.ok(modelMapper.map(reportService.getReportById(id), ReportResponseDTO.class));
    }
    @GetMapping("/location/{latitude}/{longitude}")
    public ResponseEntity<List<ReportResponseDTO>> getByLocation(@PathVariable double latitude, @PathVariable double longitude) {
        return ResponseEntity.ok(convertToListReports(reportService.getReportsByLocation(latitude, longitude)));
    }

    @GetMapping("/date/{date}")
    public ResponseEntity<List<ReportResponseDTO>> getByDate(@PathVariable LocalDateTime date) {
        return ResponseEntity.ok(convertToListReports(reportService.getReportsByDate(date)));
    }


    @PutMapping("/{id}")
    public ResponseEntity<ReportResponseDTO> put(@PathVariable int id, @RequestBody ReportRequestDTO reportRequestDTO) {
        return  ResponseEntity.ok(
                modelMapper.map(
